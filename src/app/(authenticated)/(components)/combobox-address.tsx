...
export function ComboboxAddressList({
  firm,
  defaultValue,
  onChange,
  width = "w-300px"
  ...
  const load = (s?: string) => {
    ...
    setLoading(true)
    getList(`/db/addresses?firm=${firm?._id}&pageSize=10&search=${s || search || ''}`, token)
      .then(result => {
        setList(result.docs as AddressType[])
      })
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        {!loading &&
          ...
          >
            {obj ? `${obj?.name} ${obj.cityName}` : "Seçiniz..."}

...
export default function DefinesPage() {
  ...
  const load = (pageNo?: number, s?: string) => {
    let url = `/db/itemQualities?pageSize=${pagination.pageSize}&page=${pageNo || pagination.page}`
    if (s != undefined)
  ...
  return (<>
    <div className='flex flex-col gap-4'>
      <div className='flex justify-between'>
        <h1 className='text-3xl ms-2'><i className="fa-solid fa-medal"></i> Kalite Tanımları</h1>
      ...
      {!loading && <>
        <Table className=''>
          ...
          <TableHeader >
            <TableRow >
              <TableHead colSpan={2} className="">Kalite</TableHead>
              <TableHead >Artikel</TableHead>
              <TableHead className="text-center w-14 text-2xl">
                <ButtonLink href={`/defines/itemQualities/addnew`} type={'success'}>
          ...
          <TableBody >
            {list.map(e => (
              <TableRow key={e._id}>
                <TableCell colSpan={2} className="font-medium">{e.name}</TableCell>
                <TableCell >{e.article}</TableCell>
                <TableCell className="flex justify-center gap-4 text-xl">
                  <ButtonLink href={`/defines/itemQualities/${e._id}`} type=''>

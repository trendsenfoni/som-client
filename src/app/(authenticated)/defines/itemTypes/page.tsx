...
export default function DefinesPage() {
  ...
  const load = (pageNo?: number, s?: string) => {
    let url = `/db/itemTypes?pageSize=${pagination.pageSize}&page=${pageNo || pagination.page}`
    if (s != undefined)
    ...
    <div className='flex flex-col gap-4'>
      ...
      {!loading && <>
        <Table className=''>
          ...
          <TableHeader >
            <TableRow >
              <TableHead colSpan={2} className="">Cins</TableHead>
              <TableHead >Artikel</TableHead>
              <TableHead className="text-center w-14 text-2xl">
                <ButtonLink href={`/defines/itemTypes/addnew`} type={'success'}>
            ...
            {list.map(e => (
              <TableRow key={e._id}>
                ...
                <TableCell className="flex justify-center gap-4 text-xl">
                  <ButtonLink href={`/defines/itemTypes/${e._id}`} type=''>

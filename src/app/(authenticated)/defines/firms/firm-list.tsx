...
export function FirmList({ type }: Props) {
  ...
  const load = (pageNo?: number, s?: string) => {
    let url = `/db/firms?type=${type}&pageSize=${pagination.pageSize}&page=${pageNo || pagination.page}`
  ...
  return (<>
    <div className='flex flex-col gap-4'>
      <div className='flex justify-between'>
        <h1 className='text-3xl ms-2'>
          {type == 'customer' && <><i className="fa-solid fa-building-user"></i> Müşteriler</>}
          {type == 'vendor' && <><i className="fa-solid fa-building-wheat"></i> Tedarikçiler</>}
        </h1>
              ...
              <TableRow key={e._id} className=''>
                <TableCell className="font-medium flex flex-col">
                  ...
                  <span className='text-xs text-gray-500'>
                    {e.address?.streetName} {e.address?.citySubdivisionName}
                    {e.address?.district} {e.address?.cityName}
                  </span>
                </TableCell>
                <TableCell>{e.itemArticle}</TableCell>
                <TableCell className="flex justify-center align-middle gap1-4 text-xl">
                  <ButtonLink href={`/defines/firms/${e._id}`} type=''>

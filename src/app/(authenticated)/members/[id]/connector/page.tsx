...
export default function ConnectorPage({ params }: Props) {
  ...
  const save = () => {
    putItem(`/memberSettings/${params?.id}`, token, {
      connector: connector
    })
      .then(result => {
            ...
            <div>
              {!testResult && <>
                <Button
                  ...
                  onClick={() => {
                    postItem(`/memberSettings/connectorTest`, token, { clientId: connector.clientId, clientPass: connector.clientPass })
            ...
            <div>
              {!testResult && <>
                <Button
                  variant="secondary"
                  onClick={() => {
                    postItem(`/memberSettings/mssqlTest`, token, {
  ...
  useEffect(() => {
    if (token) {
      getItem(`/memberSettings/${params?.id}`, token)
        .then(result => {
          console.log('result:', result)
          setConnector(result?.connector as ConnectorType)
          setMember(result?.member as MemberType)
        })
